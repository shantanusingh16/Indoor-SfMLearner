...
def inference(args):
    ...
    eval_split_path = '../splits/habitat_test_depth.txt'
    with open(eval_split_path, 'r') as f:
        filepaths = f.readlines()

    with torch.no_grad():
        for filepath in filepaths:

            img_path, depth_path = filepath.split()
            folder = os.path.basename(os.path.dirname(os.path.dirname(os.path.dirname(img_path))))
            fileidx = os.path.splitext(os.path.basename(img_path))[0]
            ...
            original_width, original_height = np.array(pil.open(depth_path)).shape[::-1]
            ...
            if not args.disable_median_scaling:
                gt_depth = np.array(pil.open(depth_path))
